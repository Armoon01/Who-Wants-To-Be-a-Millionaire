...
export class OpcionRespuestaService {
  private apiUrl = 'http://localhost:5255/api/opcionRespuesta'; // Ajusta la URL según tu configuración
  ...
  obtenerPorId(id: number): Observable<Opcionesrespuestas> {
    return this.http.get<Opcionesrespuestas>(`${this.apiUrl}/Obtener-por?id=${id}`)
      .pipe(catchError(this.handleError));
  }
